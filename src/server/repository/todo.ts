...
async function get({
    ...
    if (error) throw new Error("Falhou ao buscar os dados! ;/");

    const parsedData = TodoSchema.array().safeParse(data); // analisar schema oficial

    if (!parsedData.success) {
        throw new Error("Failed to parse TODO from database");
    }

    const todos = parsedData.data; // import schema que inferencia
